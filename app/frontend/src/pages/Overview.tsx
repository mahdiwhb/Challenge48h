...
export default function Overview({ onNavigate }: Props) {
  ...
  const downloadCSV = () => {
    ...
    const headers = ['Rang', 'Arrondissement', 'Score Parkshare', 'Pression Stationnement', 'Densité Résidentielle'];
    ...
    <div>
      ...
      <div className="kpi-grid">
        <div className="kpi-card">
          <div className="kpi-label">Arrondissements analysés</div>
        ...
        <div className="kpi-card success">
          ...
          <div className="kpi-value">{summary.score_moyen}<span style={{fontSize: 16}}>/100</span></div>
          <div className="kpi-sub">Potentiel Parkshare</div>
        </div>
        <div className="kpi-card success">
          <div className="kpi-label">Score maximum</div>
          <div className="kpi-value">{summary.score_max}<span style={{fontSize: 16}}>/100</span></div>
          <div className="kpi-sub">{summary.top_5[0]?.nom}</div>
        </div>
        <div className="kpi-card danger">
          <div className="kpi-label">Score minimum</div>
          <div className="kpi-value">{summary.score_min}<span style={{fontSize: 16}}>/100</span></div>
      ...
      {summary.all_scores && (
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'end', height: 200, marginTop: 12 }}>
            {summary.all_scores
              .sort((a: any, b: any) => a.rang - b.rang)
              .map((d: any) => {
                const pct = (d.score_parkshare / 100) * 100;
                const amber = d.score_parkshare >= 60 ? 'var(--status-available)' : d.score_parkshare >= 35 ? 'var(--accent)' : 'var(--status-full)';
                return (
                  <div
                  ...
                  >
                    <span style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: 'var(--text-muted)' }}>{d.score_parkshare}</span>
                    <div
                      style={{
                        width: '100%',
                        height: `${pct}%`,
                    ...
                    />
                    <span style={{ fontSize: 9, color: 'var(--text-disabled)', writingMode: 'vertical-rl', height: 40, fontFamily: "'DM Mono', monospace" }}>
